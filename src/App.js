...
const App = () => {
  async function tokenrefresh() {
    ...
    await axios.post('https://port-0-gni-server-k19y2kljzsh19o.sel4.cloudtype.app/common/token/refresh/',
      { refresh: refresh })
      .then(e => {
        localStorage.setItem('tokens', JSON.stringify({ accessToken: e.data.access, refreshToken: refresh }))
        localStorage.setItem('logintime', new Date().getTime());
        axios.defaults.headers.common['Authorization'] = e.data.access;
      })
      .catch(e => {
        console.log(e)
      })
